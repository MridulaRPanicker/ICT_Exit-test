...
model = joblib.load('linear_regression_model.joblib') # Load regression model
s_scaler = joblib.load('min_max_scaler.joblib')
label_encoder = joblib.load('label_encoder.joblib')
house_data_app = pd.read_csv('Bengaluru_House_Data.csv')
...
# Location dropdown
location_options = sorted(house_data_app['location'].dropna().unique().tolist())
...
if st.sidebar.button('Predict Price'):
    ...
    if selected_sqft <= 0 or selected_bath <= 0 or selected_bhk <= 0:
        ...
        # Features for regression model (from X in Q6) are: total_sqft, bath, balcony, bhk, 
        # area_type_Built-up  Area, area_type_Carpet  Area, area_type_Plot  Area, area_type_Super built-up  Area, location_encoded
        expected_features = ['total_sqft', 'bath', 'balcony', 'bhk', 
        ...
        if ' Area' in selected_area_type:
            area_type_col_name = f"area_type_{selected_area_type.replace(' Area', '  Area')}"
        else:
        ...
        scaled_values = s_scaler.transform(temp_df_for_scaling)
